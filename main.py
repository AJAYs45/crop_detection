...
# Setup templates and static files (Absolute Path Fix)
...
async def login_page(request: Request):
    ...
    # 🚀 FIX 4: Updated TemplateResponse signature for FastAPI 0.100+
...
async def predict_page(request: Request):
    if not is_logged_in(request): return RedirectResponse(url="/", status_code=303)
    return templates.TemplateResponse(request=request, name="predict.html", context={"request": request, "result": None})
...
async def show_history(request: Request):
    ...
    return templates.TemplateResponse(request=request, name="history.html", context={"request": request, "history": history_data})
...
async def predict_disease(
    ...
    return templates.TemplateResponse(request=request, name="predict.html", context={"request": request, "result": result_data})
